...
if prompt_yes_no "Setup WordPress configuration?"; then
	...
	sudo -u $wp_owner -i -- wp core config --dbname="$dbname" --dbuser="$dbuser" --dbpass="$dbpass" --dbhost="$dbhost" --dbprefix="$dbprefix" --extra-php --path="$wp_path" <<PHP
...
if prompt_yes_no "Do you want to install a WordPress Multisite?"; then
	...
	sudo -u $wp_owner -i -- wp core multisite-install --url="$site_url" --base=/ --title="Website" --admin_user="$admin_user" --admin_password="$admin_pass" --admin_email="admin@domain.com" --path="$wp_path" --skip-email
...
if prompt_yes_no "Install starter theme and plugins?"; then
	sudo -u $wp_owner -i -- wp plugin deactivate hello-dolly --path="$wp_path"
	sudo -u $wp_owner -i -- wp plugin delete hello-dolly --path="$wp_path"

	sudo -u $wp_owner -i -- wp theme install phlox --activate --path="$wp_path"
	sudo -u $wp_owner -i -- wp plugin install auxin-elements really-simple-ssl autoptimize duplicate-page --activate --path="$wp_path"
fi

# Enable SSL (search-replace)
if prompt_yes_no "Replace http with https in database?"; then
	sudo -u $wp_owner -i -- wp search-replace "http://" "https://"
    echo "SSL enabled."
else
